...
public class PlayerGuiController implements Initializable {
	...
	private void play() throws InterruptedException {
		...
		if (!isSingleWord() && dateList.getSelectionModel().getSelectedIndex() == 0) {
			...
			if (!setVolume) {
				...
				worker = new BashWorker("ffmpeg -y -i " + location + " -filter:a loudnorm " + output);
				Thread.sleep(200);
				worker = new BashWorker(
						"ffplay -nodisp -autoexit -af " + "silenceremove=1:0:-35dB:1:5:-35dB:0:peak '" + output + "'");
			}
		}
	}
	...
	public Task<?> createWorker2() {
		return new Task<Object>() {
			...
			protected Object call() throws Exception {
				System.out.println("we doin");
				List<String> stringList = new ArrayList<>();
				for (int i = 0; i < nameArray.length; i++) {
					File file = fileManager.getRandomGoodFile(nameArray[i]);
					String location = file.toURI().toString();
					stringList.add("file " + "'" + location.substring(5) + "'");
				}
				FileLogger.getInstance().writeToFile("Logs/toParse.txt", stringList);
				worker = new BashWorker("ffmpeg -y -f concat -safe 0 -i toParse.txt -c:a copy  output.wav "
						+ " && ffmpeg -i output.wav -af silenceremove=1:0:-20dB output1.wav" + " && ffplay "
						+ "-autoexit -nodisp -autoexit -af output1.wav");
				return true;
			}
		};
	}
			...
			protected Object call() throws Exception {
				...
				for (int i = 0; i < nameArray.length; i++) {
					try {
						...
						location = location.replace("%20", " ");
						String output = "se206_output.wav";
						String time = "se206_time.wav";
						worker = new BashWorker("ffmpeg -y -i " + location + " -filter:a loudnorm " + output);
						worker = new BashWorker("ffmpeg  -y -i " + location + " -af "
								+ "silenceremove=1:0:-35dB:1:5:-35dB:0:peak '" + time + "'");
						Thread.sleep(200);
						...
						if (!setVolume) {
							...
							int time1 = (int) (durationInSeconds * 1000);
							Thread.sleep(time1);
						} else {
							worker = new BashWorker("ffplay -nodisp -autoexit -af "
									+ "silenceremove=1:0:-35dB:1:5:-35dB:0:peak '" + output + "'");
							File TemporaryFileTime = new File(time);
							File TemporaryFile = new File(output);
                            
                            AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(TemporaryFileTime);
							AudioFormat format = audioInputStream.getFormat();
							long frames = audioInputStream.getFrameLength();
							double durationInSeconds = (frames + 0.0) / format.getFrameRate();
							int time1 = (int) (durationInSeconds * 1000);
	...
	private void stop() {
		...
		File temporareFile = new File("se206_output.wav");
		if (temporareFile.exists()) {
		   temporareFile.delete();
		}
	}
	...
	/**
	 * Check if a recording has been marked as bad and throw an error message if
	 * required
	 */
	@FXML
